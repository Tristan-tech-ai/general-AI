...
def bangun():
    ...
    E.append(P(
        ...
        "satu pun. Angka tersebut merupakan hitungan berkas langsung dan tidak "
        "bergantung pada pengujian statistik apa pun.", "p"))
    E.append(P(
        "Setiap perbandingan dalam penelitian ini kemudian dijalankan dengan tiga "
        "sampai lima inisialisasi acak per sel, diuji dengan uji t Welch, dan "
        "dikoreksi Holm-Bonferroni. Dari seluruh perbandingan itu, dua keputusan "
        "terbukti menentukan dan keduanya tidak terletak pada arsitektur.", "p"))
    E.append(P(
        "Keputusan pertama adalah penetapan ambang keputusan. Pada partisi resmi, "
        "model yang sama memisahkan kedua kelas dengan area under curve 0,9756 "
        "namun hanya mencapai 50,03 persen akurasi pada ambang tetap 0,5, "
        "sedangkan pada ambang yang disesuaikan dengan prior kelas mencapai 92,56 "
        "persen. Selisih 42,52 poin persentase itu sepenuhnya merupakan kegagalan "
        "kalibrasi dan bukan kegagalan pengenalan. Pola yang sama muncul pada "
        "eksperimen yang sepenuhnya terpisah, yaitu pengujian di bawah gangguan "
        "noise, ketika WavLM mempertahankan area under curve 0,9615 pada 10 dB "
        "tetapi hanya mencapai 59,8 persen pada ambang beku, dan penyesuaian "
        "ambang memulihkan 29,6 poin. Dua eksperimen dengan arsitektur dan kondisi "
        "yang berbeda karena itu menunjuk sebab yang sama.", "p"))
    E.append(P(
        "Keputusan kedua adalah besaran learning rate relatif terhadap ukuran dan "
        "jenis encoder. Learning rate seragam 0,001 yang ditetapkan rencana awal "
        f"memberi {_sn(A['ast_proposal'])} pada AST yang berukuran 86 juta "
        f"parameter, tetapi menjatuhkan WavLM Large ke {_sn(A['wavlm_proposal'])} "
        f"dan HuBERT Large ke {_sn(A['hubert_proposal'])}, yaitu mendekati tebakan "
        "acak. Kekeliruan pada keputusan ini tidak dapat ditambal oleh perbaikan "
        "lain mana pun di dalam pipeline.", "p"))
    E.append(P(
        "Sebaliknya, sejumlah pernyataan yang semula dilaporkan sebagai temuan "
        "tidak bertahan setelah ragam antar inisialisasi diperhitungkan. Yang "
        "termasuk di dalamnya adalah korelasi negatif antara akurasi in-domain dan "
        "kemampuan generalisasi, keunggulan augmentasi band-gain yang diusulkan "
        "penelitian ini sendiri, dan perbandingan antara membekukan dan melatih "
        "encoder pada ketiga arsitektur. Lampiran A memuat daftar lengkapnya "
        "beserta alasan penarikan masing-masing. Aturan yang muncul dari data "
        "sendiri adalah bahwa pada anggaran komputasi penelitian ini, hanya efek "
        "berpuluh poin persentase yang dapat dibuktikan, sedangkan efek berbilang "
        "poin tidak.", "p"))
    ...
    E.append(P(
        ...
        "selisih angka yang dilaporkan dapat ditelusuri ke penetapan ambang "
        "keputusan dan ke artefak kompresi yang berkorelasi dengan label. "
        "Kegagalan di bawah gangguan noise juga sebagian besar merupakan kegagalan "
        "kalibrasi ambang, bukan kegagalan pengenalan, sehingga dapat dipulihkan "
        "tanpa mengubah model. Kedua pengamatan itu semula dilaporkan sebagai dua "
        "temuan yang terpisah, dan baru setelah selisih pada temuan pertama "
        "dipecah menjadi sebab-sebabnya terlihat bahwa keduanya menunjuk mekanisme "
        "yang sama. Penyatuan itu tidak direncanakan dan justru muncul dari upaya "
        "menarik kembali klaim yang terlalu kuat. "
